...
df_ = pd.read_csv(path + 'Predictions_singleKw.csv')
df = df_.set_index('date').T.reset_index().round(1)
df_to_decompose = df_.iloc[:-6, :]

df_groups_ = pd.read_csv(path + 'Predictions_groupedKw.csv')
...
groups = pd.read_csv('Predictions Galp/kw_groups.csv')
    ...
    for key, value in dict_groups.items():
        if group == key:
            if group == 'All keyword groups':
                ...
                df1 = df[df['index'].isin(ast.literal_eval(value))]
